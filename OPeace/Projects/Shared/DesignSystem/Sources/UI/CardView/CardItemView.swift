...
public struct CardItemView: View {
  ...
  private var resultData: QuestionContentData
  ...
  public init(
    resultData: QuestionContentData,
    ...
    _answerRatio = State(initialValue: (Int(resultData.answerRatio?.answerRatioA ?? .zero), Int(resultData.answerRatio?.answerRatioB ?? .zero)))
      ...
      RoundedRectangle(cornerRadius: 30)
        ...
        .overlay {
          VStack {
            ...
            isVotedResultView(
              responseCount: resultData.answerCount,
              likeCount: resultData.likeCount
            )
            Spacer()
          }
        ...
        if resultData.metadata?.liked == true {
          ...
          .onTapGesture {
            if !isUserInteractionDisabled {
              if userLoginID != resultData.userInfo?.userID {
                isLikedTap.toggle()
                likeTapAction(String(resultData.id))
              }
